...
- [ ] Инициализировать React + TypeScript проект через Vite
  - **Шаг 1:** Выполнить команду в терминале: `npm create vite@latest duck-game -- --template react-ts`
  - **Шаг 2:** Перейти в созданную директорию: `cd duck-game`
  - **Шаг 3:** Установить зависимости: `npm install`
  - **Шаг 4:** Проверить, что проект запускается: `npm run dev`
  - **Шаг 5:** Открыть браузер и убедиться, что приложение работает на `http://localhost:5173`
  - **Шаг 6:** Остановить dev-сервер (Ctrl+C) и приступить к настройке структуры
...
- [ ] При обнаружении коллизии:
  ...
  - **Шаг 4:** Добавить визуальную индикацию столкновения (опционально: мигание экрана)
