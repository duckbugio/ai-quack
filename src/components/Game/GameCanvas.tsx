...
import { BackgroundRenderer } from '../../game/utils/BackgroundRenderer';
import { DecorationsManager } from '../../game/utils/DecorationsManager';
...
export const GameCanvas: React.FC<GameCanvasProps> = ({ 
  ...
  const decorationsManagerRef = useRef<DecorationsManager | null>(null);
  ...
  if (!decorationsManagerRef.current) {
    decorationsManagerRef.current = new DecorationsManager();
  }
  ...
  useEffect(() => {
    if (decorationsManagerRef.current) {
      decorationsManagerRef.current.initialize(width, height);
    }
  }, [width, height]);
  ...
  const updateBackground = useCallback(
    (deltaTime: number, speedMultiplier: number = 1) => {
      if (!backgroundRendererRef.current || !decorationsManagerRef.current) return;
      
      backgroundRendererRef.current.updateClouds(deltaTime, width);
      backgroundRendererRef.current.updateGround(deltaTime, width, speedMultiplier);
      
      const trees = decorationsManagerRef.current.getTrees();
      backgroundRendererRef.current.updateTrees(deltaTime, width, trees);
      
      decorationsManagerRef.current.updateBirds(deltaTime, width, height);
    },
    [width, height]
  );
  ...
  const update = useCallback(
    (deltaTime: number) => {
      ...
      updateBackground(deltaTime, difficultyMultiplier);
    ...
    [gameState, height, score, checkCollisions, gameOver, updateBackground]
  );
  ...
  const render = useCallback(() => {
    ...
    backgroundRendererRef.current.drawSky(ctx, width, height);
    backgroundRendererRef.current.drawClouds(ctx, width);
    
    const trees = decorationsManagerRef.current.getTrees();
    backgroundRendererRef.current.drawTrees(ctx, width, height, trees);
    
    backgroundRendererRef.current.drawGround(ctx, width, height);
    
    const flowers = decorationsManagerRef.current.getFlowers();
    backgroundRendererRef.current.drawFlowers(ctx, width, height, flowers);
    
    const birds = decorationsManagerRef.current.getBirds();
    backgroundRendererRef.current.drawBirds(ctx, width, birds);
  ...
  useEffect(() => {
    if (gameState === GameState.MENU) {
      ...
      if (decorationsManagerRef.current) {
        decorationsManagerRef.current.reset();
      }
    }
  }, [gameState]);
