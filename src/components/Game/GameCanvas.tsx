...
import { DecorativeElementsManager } from '../../game/systems/DecorativeElementsManager';
...
export const GameCanvas: React.FC<GameCanvasProps> = ({ 
  ...
  const decorativeElementsRef = useRef<DecorativeElementsManager | null>(null);
  ...
  if (!decorativeElementsRef.current) {
    decorativeElementsRef.current = new DecorativeElementsManager(width, height);
  }
  ...
  const update = useCallback(
    (deltaTime: number) => {
      ...
      if (decorativeElementsRef.current) {
        decorativeElementsRef.current.update(deltaTime, difficultyMultiplier);
      }
  ...
  const render = useCallback(() => {
    ...
    if (!backgroundRendererRef.current || !decorativeElementsRef.current) return;
    ...
    backgroundRenderer.drawClouds(ctx, decorativeElements.getCloudOffset());
    backgroundRenderer.drawTrees(ctx, decorativeElements.getTrees(), decorativeElements.getTreesOffset());
    backgroundRenderer.drawGround(ctx, decorativeElements.getGroundOffset());
    backgroundRenderer.drawFlowers(ctx, decorativeElements.getFlowers(), decorativeElements.getGroundOffset());
    backgroundRenderer.drawBirds(ctx, decorativeElements.getBirds());
    ...
    if (gameState === GameState.MENU || gameState === GameState.GAME_OVER || gameState === GameState.PAUSED) {
      ...
      if (gameState === GameState.MENU || gameState === GameState.GAME_OVER) {
        ...
        const animateBackground = (currentTime: number) => {
          ...
          if (decorativeElementsRef.current) {
            decorativeElementsRef.current.update(deltaTime, 1);
          }
          renderRef.current();
  ...
  useEffect(() => {
    if (gameState === GameState.MENU) {
      ...
      if (decorativeElementsRef.current) {
        decorativeElementsRef.current.reset();
      }
    }
  }, [gameState]);
