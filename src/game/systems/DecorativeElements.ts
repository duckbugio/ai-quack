...
export class DecorativeElements {
  ...
  constructor(canvasWidth: number, canvasHeight: number) {
    ...
    this.initializeFlowers(canvasHeight);
  }
  ...
  private initializeFlowers(groundY: number): void {
    this.flowers = [
  ...
  updateClouds(deltaTime: number, canvasWidth: number): void {
    this.cloudOffset += 0.1 * (deltaTime / 16);
    if (this.cloudOffset > canvasWidth) {
      this.cloudOffset = 0;
    }
  }

  updateGround(deltaTime: number, speedMultiplier: number, canvasWidth: number): void {
    ...
    if (this.groundOffset > canvasWidth) {
      this.groundOffset = 0;
    }
  }

  updateTrees(deltaTime: number, canvasWidth: number): void {
    const GROUND_SPEED = 3;
    ...
    this.trees.forEach((tree) => {
      ...
      if (treeScreenX + 100 < -canvasWidth) {
    ...
    if (this.treesOffset > canvasWidth * 2) {
      this.treesOffset = 0;
    }
  }

  updateBirds(deltaTime: number, canvasWidth: number, canvasHeight: number): void {
